...
function shallowCopy(obj) {
  ...
  Object.keys(obj).forEach((el) => {
    if (typeof el !== 'object') res[el] = obj[el];
    else {
      res[el] = Object.assign(obj);
    }
  });
  return res;
}
...
function compareObjects(obj1, obj2) {
  try {
    const res = Object.entries(obj1).reduce((acc, key) => {
      if (key[1] === acc[key[0]]) delete acc[key[0]];
      else throw Error('Some');
      return acc;
    }, Object.assign(obj2));
...
function sortCitiesArray(arr) {
  arr.sort((a, b) => {
    let comparation = 0;
    if (a.country < b.country) comparation = -1;
    if (a.country > b.country) comparation = 1;
    if (comparation !== 0) return comparation;
    let comparationCity = 0;
    if (a.city < b.city) comparationCity = -1;
    if (a.city > b.city) comparationCity = 1;
    return comparationCity;
  });
  return arr;
}
/**
...
const cssSelectorBuilder = {
  ...
  pseudoClass(value) {
    this.selector += `:${value}`;
    return new CustomBuilder().pseudoClass(value);
  },

  pseudoElement(value) {
    this.selector += `::${value}`;
    return new CustomBuilder().pseudoElement(value);
  },
